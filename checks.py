...
«Высокая уверенность» на дате из будущего не отменяет ничего — и низкая на
целой строке ничего не портит: её слышно предупреждением, а статус ставят
проверки.
"""
...
def review(answer, categories, today):
    ...
    # Своя неуверенность агента человеку интересна, но статуса не меняет.
    # Ресторанный чек 3 сентября: сумма 205 на месте, дата читается, статья
    # «кафе и рестораны» очевидна по составу заказа — а «проверить» встало
    # только потому, что агент приуныл от нечитаемой вывески и
    # непропечатанной валюты. Звать человека проверять такую строку не за
    # чем: всё, что и правда её портит, ловится проверками выше — не
    # прочиталась сумма, агент обязан прислать null; не прочиталась дата —
    # пустую строку. Иначе статус строки висит на настроении модели.
    if answer.get("confidence") == "низкая":
        warnings.append("агент не уверен в прочитанном")
