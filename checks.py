...
def review(answer, categories, today):
    ...
    # Колонка называется «продавец / на что». Пустая — и строка в таблице
    # ничего не говорит: «2 сентября, 12000, Прочее». Такую трату не найти
    # и не оспорить через месяц. Проверка на пустоту здесь потому, что
    # иначе статус этой строки висит на confidence агента — на том самом
    # поле, которому эта таблица не верит нигде больше.
    if not (answer.get("merchant") or "").strip():
        reasons.append("не понятно, за что платили")
