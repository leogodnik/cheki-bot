...
(function () {
  ...
  function receive(answer) {
    ...
    /* Пустое начало и лента — одно место в двух состояниях, и переключать их
       ...
       «Доступа» обратно в ленту каждые три секунды, посреди
       набранного токена. */
  ...
  /* Статью в справочник дописывают в таблице, а не здесь, и узнать об этом
     можно только у таблицы. Удобный момент один: человек вернулся во вкладку —
     ходил он, скорее всего, как раз туда. Сервер перечитает справочник в фоне,
     новое число в пункте «Таблица» приедет ближайшим опросом.

     Отдельным запросом, а не внутри poll: опрос ходит раз в три секунды,
     Apps Script отвечает за двадцать, и вешать чтение на каждый заход значило
     бы держать очередь запросов к таблице ради числа, которое меняется раз
     в месяц. Сам опрос справочник тоже освежает, но по концу кэша — этого
     хватает боту и не хватает человеку, стоящему над сайдбаром. */
  document.addEventListener("visibilitychange", function () {
    if (document.visibilityState === "visible") {
      post("/api/sheet/reread", {}).catch(function () {});
    }
  });
