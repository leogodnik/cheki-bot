...
(function () {
  ...
  /* «Смотрю чек…» на фотографию, «Разбираю…» на текст — плашка на 15–50
     секунд в обоих случаях: текст читает haiku и обычно укладывается быстрее,
     но замеров нет, а обещать «быстрее» и не успеть хуже честной верхней
     границы. Её уберёт событие с тем же номером задачи.

     Событие без поля job — старая лента с прошлого запуска бота (странице
     мог достаться журнал целиком после перезапуска): тогда, как и раньше,
     «Смотрю чек…». */
  function running(event) {
    ...
    var hdr = el("div", "hdr");
    hdr.appendChild(icon("mark"));
    var word = event.job === "текст" ? "Разбираю… " : "Смотрю чек… ";
    var run = el("span", "run", word);
