...
(function () {
  ...
  function receive(answer) {
    ...
    /* У каждого запуска бота своя метка life; сменилась — прошлая жизнь
       нарисовала то, чего сервер больше не помнит, — стираем нарисованное.
       Курсор last здесь не трогаем: за меткой такого же мнения и опрос — он
       уезжает со своей же life (см. poll()), сервер по ней узнаёт старую
       страницу и в этом самом ответе присылает журнал заново целиком, а
       Math.max ниже сам доведёт курсор до конца этого журнала. Первый ответ
       в жизни страницы метку просто запоминает, ничего не стирая. */
