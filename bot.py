...
def poll_with(env, st, jobs, token):
    ...
    while True:
        try:
            ...
            time.sleep(5)
            # Та же проверка, что ниже, и по той же причине — но здесь она
            # нужна отдельной строкой: до нижней виток не доходит, он уходит
            # на continue. Без неё замена бота при молчащем телеграме тонула
            # молча: токен в .env новый, ник в сайдбаре новый, точка зелёная,
            # а опрос до самого перезапуска долбится в прежний токен, и новому
            # боту не отвечает никто. А молчит телеграм ровно тогда, когда бота
            # и меняют: токен отозвали в @BotFather, и getUpdates отдаёт 401.
